...
contract FlightSuretyApp {
    ...
    struct ResponseInfo {
        ...
        bool isOpen;                                    // If already open, oracle responses are accepted
        mapping(uint8 => address[]) responses;          // Mapping key is the status code reported .
    }

    mapping(address => Oracle) private oracles;                 // Track all the registered oracles
    mapping(bytes32 => ResponseInfo) private oracleResponses;   // Track all the oracle responses: Key = hash(index, flight, timestamp)
