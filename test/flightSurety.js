...
contract('Flight Surety Tests', async (accounts) => {
    ...
    it("Only existing airline may register a new airline until there are at least four airlines registered", async () => {
        const Acc_Offset = 4; // start with 3 because  1 and 2 are already in use (use clean address)
        const max_airlines = 2; // four minus two which are already registered

        for (let i = 0; i < max_airlines; ++i) {
            try {
                await config.flightSuretyApp.sendTransaction({from: accounts[i + Acc_Offset], value: INITIAL_FUND});
                await config.flightSuretyApp.registerAirline("My Airline", accounts[i + Acc_Offset], {from: config.firstAirline});
            } catch (e) {
                console.log(e)
            }
            let result = await config.flightSuretyData.isAirlineRegistered.call(accounts[i + Acc_Offset]);
    ...
    it("Registration of fifth and subsequent airlines requires multi-party consensus of 50% of registered airlines", async () => {
        const Acc_Offset = 6; // Acc_Offset + max_airlines of previous test (aligned)
        const vote_offset = 4; // Acc_Offset of previous test
        ...
        for (let i = 0; i < max_airlines; ++i) {
            ...
            for (let k = 0; k < votes_needed; ++k) {
                try {
                    await config.flightSuretyApp.registerAirline("My Airline", accounts[i + Acc_Offset], {from: accounts[k + vote_offset]});
                } catch (e) {
                    console.log(e)
                }
                let result = await config.flightSuretyData.isAirlineRegistered.call(accounts[i + Acc_Offset]);
    ...
    it("Passenger can withdraw any funds owed to them as a result of receiving credit for insurance payout", async () => {
        let customer = accounts[59];
        let remaining_bal = web3.utils.fromWei(await web3.eth.getBalance(customer), 'ether');
        let Fund_remaining = web3.utils.fromWei(await config.flightSuretyData.checkFunds(customer), 'ether');
        let tx = await config.flightSuretyApp.getFunds({from: customer});
        let New_Bal = web3.utils.fromWei(await web3.eth.getBalance(customer), 'ether');

        console.log("The Passenger fund is %d",Fund_remaining);
        console.log("The Balance is %d", remaining_bal);
        console.log("The New Balance is %d", New_Bal);
        console.log("The Withdrew %d", New_Bal - remaining_bal);

        // To compare result, change variable type from string to float so it can return bool properly
        assert.equal(parseFloat(New_Bal) > parseFloat(remaining_bal), true, 'New balance should be bigger');
    });
});
